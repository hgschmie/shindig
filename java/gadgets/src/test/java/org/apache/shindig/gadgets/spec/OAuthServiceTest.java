...
public class OAuthServiceTest {
  ...
  public void testParseAuthorizeUrl() throws Exception {
    ...
    OAuthAuthorization authorization = StaxTestUtils.parseElement(xml,
        new OAuthAuthorization.Parser(SPEC_URL));
  ...
  public void testParseEndPoint() throws Exception {
    ...
    OAuthElement request = StaxTestUtils.parseElement(xml,
        new OAuthRequest.Parser(SPEC_URL));
  ...
  public void testParseService() throws Exception {
    String xml = "" + "<Service name='thename'>"
        ...
        + "</Service>";
    OAuthService s = StaxTestUtils.parseElement(xml, new OAuthService.Parser(
        SPEC_URL));
    assertEquals("thename", s.getName());
    assertEquals(OAuthElement.Location.HEADER, s.getAccess().getParamLocation());
    assertEquals(Uri.parse("http://azn.example.com/quux"), s.getAuthorization()
        .getUrl());
  }
  ...
  public void testParseService_reqonly() throws Exception {
    String xml = "<Service>"
        + "<Request url='http://www.example.com/request'/>" + "</Service>";
    StaxTestUtils.parseElement(xml, new OAuthService.Parser(SPEC_URL));
  }
  ...
  public void toStringIsSane() throws Exception {
    ...
    OAuthElement request = StaxTestUtils.parseElement(xml2,
        new OAuthRequest.Parser(SPEC_URL));
    assertEquals(Uri.parse("http://www.example.com"), request.getUrl());
    assertEquals(OAuthElement.Location.BODY, request.getParamLocation());
    assertEquals(OAuthElement.Method.POST, request.getMethod());

    OAuthAuthorization authorization2 = StaxTestUtils.parseElement(
        authorization.toString(), new OAuthAuthorization.Parser(SPEC_URL));
    assertEquals(authorization.getUrl(), authorization2.getUrl());

    OAuthElement request2 = StaxTestUtils.parseElement(request.toString(),
        new OAuthRequest.Parser(SPEC_URL));
