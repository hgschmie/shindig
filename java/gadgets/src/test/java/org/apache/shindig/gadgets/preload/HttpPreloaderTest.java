...
public class HttpPreloaderTest {
  ...
  private static final Uri GADGET_URL = Uri
      .parse("http://example.org/gadget.xml");
  private static final Map<String, String> PRELOAD_METADATA = Maps
  ...
  private final ContentFetcherFactory fetchers = new ContentFetcherFactory(
      null, null) {
    @Override
  ...
  private void checkRequest(HttpRequest request) {
    ...
    assertEquals(context.getToken().getAppId(), request.getSecurityToken()
        .getAppId());
  }
  ...
  private static void checkResults(JSONObject results) throws JSONException {
    ...
    assertEquals("yo=momma", results.getJSONObject("headers").getJSONArray(
        "set-cookie").get(0));

    for (Entry<String, String> entry : PRELOAD_METADATA.entrySet()) {
      assertEquals("Metadata values not copied to output.", entry.getValue(),
  ...
  public void normalPreloads() throws Exception {
    String xml = "<Module><ModulePrefs title=''>" + " <Preload href='"
        + PRELOAD_HREF + "'/>" + "</ModulePrefs><Content/></Module>";
    GadgetSpec gadget = StaxTestUtils.parseSpec(xml, GADGET_URL);
    Preloader preloader = new HttpPreloader(fetchers);

    Map<String, Callable<PreloadedData>> preloaded = preloader
        .createPreloadTasks(context, gadget);
  ...
  public void signedPreloads() throws Exception {
    String xml = "<Module><ModulePrefs title=''>" + " <Preload href='"
        + PRELOAD_HREF + "' authz='signed' sign_viewer='false'/>"
  ...
  public void oauthPreloads() throws Exception {
    String xml = "<Module><ModulePrefs title=''>"
        +
  ...
  public void multiplePreloads() throws Exception {
    String xml = "<Module><ModulePrefs title=''>" + " <Preload href='"
        + PRELOAD_HREF + "'/>" + " <Preload href='" + PRELOAD_HREF2 + "'/>"
        + "</ModulePrefs><Content/></Module>";
    GadgetSpec gadget = StaxTestUtils.parseSpec(xml, GADGET_URL);
    Preloader preloader = new HttpPreloader(fetchers);
  ...
  public void onlyPreloadForCorrectView() throws Exception {
    String xml = "<Module><ModulePrefs title=''>" + " <Preload href='"
        + PRELOAD_HREF + "' views='foo,bar,baz'/>" + " <Preload href='"
        + PRELOAD_HREF2 + "' views='bar'/>"
        + "</ModulePrefs><Content/></Module>";
    GadgetSpec gadget = StaxTestUtils.parseSpec(xml, GADGET_URL);
    Preloader preloader = new HttpPreloader(fetchers);
    ...
    Map<String, Callable<PreloadedData>> preloaded = preloader
        .createPreloadTasks(fooViewContext, gadget);

    PreloadedData data = preloaded.get(PRELOAD_HREF).call();
    checkRequest(plainFetcher.requests.get(0));
    checkResults((JSONObject) data.toJson());

    assertNull("Preloaded an item that should not have been.", preloaded
        .get(PRELOAD_HREF2));
  }
  ...
  private static class RecordingHttpFetcher implements HttpFetcher {
    ...
    public HttpResponse fetch(HttpRequest request) {
      ...
      return new HttpResponseBuilder().setMetadata(PRELOAD_METADATA)
          .setResponseString(PRELOAD_CONTENT).addHeader("Set-Cookie",
              "yo=momma").create();
    }
  }
}
